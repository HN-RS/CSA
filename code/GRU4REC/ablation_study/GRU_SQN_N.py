...
def double_qlearning_(q_tm1, a_tm1, r_t, pcont_t, q_t_value, q_t_selector, args, name="DoubleQLearning"):
    # Rank and compatibility checks.
    base_ops.wrap_rank_shape_assert(
        [[q_tm1, q_t_value, q_t_selector], [a_tm1, r_t, pcont_t]], [2, 1], name)
    # double Q-learning op.
    with tf.name_scope(name, values=[q_tm1, a_tm1, r_t, pcont_t, q_t_value, q_t_selector]):
        # Build target and select head to update.
        q_tm1_ = tf.reshape(q_tm1, [args.num_aug + 1, -1, args.item_num])
        q_t_value_ = tf.reshape(q_t_value, [args.num_aug + 1, -1, args.item_num])
        q_t_selector_ = tf.reshape(q_t_selector, [args.num_aug + 1, -1, args.item_num])

        double_q_bootstrapped_tmp = [[]] * (args.num_aug + 1)
        qa_tm1_tmp = [[]] * (args.num_aug + 1)
        best_action = [[]] * (args.num_aug + 1)
        for i in range(args.num_aug + 1):
            best_action[i] = tf.argmax(q_t_selector_[i], 1, output_type=tf.int32)
            double_q_bootstrapped_tmp[i] = indexing_ops.batched_index(q_t_value_[i], best_action[i])

            qa_tm1_tmp[i] = indexing_ops.batched_index(q_tm1_[i], a_tm1)

        double_q_bootstrapped = tf.reduce_mean(double_q_bootstrapped_tmp, 0)
        qa_tm1 = tf.reduce_mean(qa_tm1_tmp, 0)
...
class QNetwork:
    def __init__(self, hidden_size, learning_rate, item_num, state_size, pretrain, args, name='DQNetwork'):
        ...
        with tf.variable_scope(self.name):
            ...
            self.loss = tf.reduce_mean(self.qloss + args.q_w*tf.reduce_mean(self.celoss, 0))
    ...
    with tf.Session(config=tf.ConfigProto(gpu_options=gpu_options)) as sess:
        ...
        for i in range(args.epoch):
            for j in range(num_batches):
                ...
                # qloss,  celoss,celoss_ = sess.run([mainQN.qloss, mainQN.celoss, mainQN.celoss_],
                ...
                # # print("qloss:", qloss)
                # print("celoss:", celoss.shape)
                # # print("celoss:", celoss)
                # print("celoss_:", celoss_.shape)
