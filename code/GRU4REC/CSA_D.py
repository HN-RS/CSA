...
def parse_args():
    ...
    parser.add_argument('--p_drop', type=float, default=0.1,
                        help='probability of dimension drop.')
...
class QNetwork:
    def __init__(self, hidden_size, learning_rate, item_num, state_size, pretrain, args, name='DQNetwork'):
        ...
        with tf.variable_scope(self.name):
            ...
            self.noise = tf.placeholder(tf.float32, [args.num_aug, None, self.hidden_size], name='noise')
            ...
            self.output_neg = tf_slim.fully_connected(self.states_hidden_neg, self.item_num,
                                                      ...
                                                      reuse=tf.AUTO_REUSE)  # all q-values
...
if __name__ == '__main__':
    ...
    # train_noise = tfp.distributions.Bernoulli(probs=0.4)
    train_noise = np.ones([args.num_aug, args.batch_size, args.hidden_factor])
            ...
            for j in range(num_batches):
                ...
                for na in range(args.num_aug):
                    for bat in range(args.batch_size):
                        ...
                        x = np.random.randint(args.hidden_factor)
                        #
                        prob = np.random.random()
                        if prob < args.p_drop:
                            train_noise[na][bat][x] = 0
